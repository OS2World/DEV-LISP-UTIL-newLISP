...
/* 
   ...
   (bayes-query L1 L2 [L3 ... LN] D [trueChainedBayes])
   ...
   Token can be strings or symbols. I token are strings they are prepended with an
   underscore when looked up in D. When frequencires in D wehere learned using 
   bayes-train, the underscore was utomatically prepended during learning.
   ...
   When in chain Bayes mode p(Mcat|tkn) is the posterior for Mcat and replaces
   the pior p(Mcat) for the next token. In chain Bayes mode tokens with 0 frequency 
   in one catgory will effectiviely put the probability ofthis category to 0. This causes
   queries resulting in 0 probkabilities for all categories to yield NaN values. 
...
/* get number of categories maxIdx */
